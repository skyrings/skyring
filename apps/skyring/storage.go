					...
					for {
						...
						if providerTask.Completed {
							if providerTask.Status == models.TASK_STATUS_SUCCESS {
								...
								if err := logAuditEvent(EventTypes["STORAGE_CREATED"],
									fmt.Sprintf("Created storage:%s for cluster: %v", request.Name, cluster_id_str),
									fmt.Sprintf("Created storage:%s for cluster: %s", request.Name, cluster_id_str),
									cluster_id,
									models.NOTIFICATION_ENTITY_STORAGE,
									&(t.ID),
									ctxt); err != nil {
...
func (a *App) DEL_Storage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Get().Error("%s - Error parsing the storage id: %s. error: %v", ctxt, storage_id_str, err)
		if err := logAuditEvent(EventTypes["STORAGE_DELETED"],
			fmt.Sprintf("Failed to delete storage:%v for cluster: %v", storage_id_str, cluster_id_str),
			fmt.Sprintf("Failed to delete storage:%v for cluster: %s. Error: %v", storage_id_str, cluster_id_str, err),
			nil,
			models.NOTIFICATION_ENTITY_STORAGE,
			nil,
			ctxt); err != nil {
					...
					for {
						...
						if providerTask.Completed {
							if providerTask.Status == models.TASK_STATUS_SUCCESS {
								t.UpdateStatus("Success")
								if err := logAuditEvent(EventTypes["STORAGE_DELETED"],
									fmt.Sprintf("Deleted storage:%v for cluster: %v", storage_id_str, cluster_id_str),
									fmt.Sprintf("Deleted storage:%v for cluster: %v", storage_id_str, cluster_id_str),
									storage_id,
									models.NOTIFICATION_ENTITY_STORAGE,
									&(t.ID),
...
func (a *App) PATCH_Storage(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		logger.Get().Error(
			...
			err)
		if err := logAuditEvent(EventTypes["STORAGE_UPDATED"],
			fmt.Sprintf("Failed to update storage:%s for cluster: %v", storage_id_str, cluster_id_str),
			fmt.Sprintf("Failed to update storage:%s for cluster: %s. Error: %v", storage_id_str, cluster_id_str, err),
			nil,
			models.NOTIFICATION_ENTITY_STORAGE,
			nil,
			ctxt); err != nil {
				...
				for {
					...
					if providerTask.Completed {
						if providerTask.Status == models.TASK_STATUS_SUCCESS {
							t.UpdateStatus("Success")
							if err := logAuditEvent(EventTypes["STORAGE_UPDATED"],
								fmt.Sprintf("Updated storage:%s for cluster: %v", storage_id_str, cluster_id_str),
								fmt.Sprintf("Updated storage:%s for cluster: %s", storage_id_str, cluster_id_str),
								storage_id,
								models.NOTIFICATION_ENTITY_STORAGE,
								&(t.ID),
								ctxt); err != nil {
