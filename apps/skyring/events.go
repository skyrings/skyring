...
func GetEvents(rw http.ResponseWriter, req *http.Request) {
	...
	if len(severity) != 0 {
		...
		case "indeterminate":
			filter["severity"] = models.ALARM_STATUS_INDETERMINATE
		case "critical":
			filter["severity"] = models.ALARM_STATUS_CRITICAL
		case "major":
			filter["severity"] = models.ALARM_STATUS_MAJOR
		case "minor":
			filter["severity"] = models.ALARM_STATUS_MINOR
		case "warning":
			filter["severity"] = models.ALARM_STATUS_CRITICAL
		case "cleared":
			filter["severity"] = models.ALARM_STATUS_CLEARED
		default:
...
func GetEventFromId(w http.ResponseWriter, r *http.Request) {
