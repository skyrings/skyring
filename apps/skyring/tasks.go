...
func (a *App) getTasks(rw http.ResponseWriter, req *http.Request) {
	...
	tasks := task.GetManager().List()
	for _, taskId := range tasks {
		started, _ := task.GetManager().IsStarted(taskId)
		completed, _ := task.GetManager().IsDone(taskId)
		statusList, _ := task.GetManager().GetStatus(taskId)
