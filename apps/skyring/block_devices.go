...
func (a *App) POST_BlockDevices(w http.ResponseWriter, r *http.Request) {
	...
	if taskId, err := a.GetTaskManager().Run(models.ENGINE_NAME, fmt.Sprintf("Create Block Device: %s", request.Name), asyncTask, 300*time.Second, nil, nil, nil); err != nil {
...
func (a *App) DELETE_BlockDevice(w http.ResponseWriter, r *http.Request) {
	...
	if taskId, err := a.GetTaskManager().Run(models.ENGINE_NAME, fmt.Sprintf("Delete Block Device: %v", *blockdevice_id), asyncTask, 300*time.Second, nil, nil, nil); err != nil {
...
func (a *App) PATCH_ResizeBlockDevice(w http.ResponseWriter, r *http.Request) {
	...
	if taskId, err := a.GetTaskManager().Run(models.ENGINE_NAME, fmt.Sprintf("Resize Block Device: %v", *blockdevice_id), asyncTask, 300*time.Second, nil, nil, nil); err != nil {
