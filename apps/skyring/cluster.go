...
func writeCreateResponse(w http.ResponseWriter, request models.StorageCluster, result []byte) {
	...
	if status != http.StatusOK {
		...
		for _, node := range request.Nodes {
			...
			if err := coll.Update(bson.M{"hostname": node}, bson.M{"$set": bson.M{"clusterid": request.ClusterId, "managedstate": models.NODE_STATE_USED}}); err != nil {
				util.HttpResponse(w, http.StatusInternalServerError, err.Error())
...
func writeExpandResponse(w http.ResponseWriter, request models.StorageCluster, result []byte) {
	var m map[string]interface{}
	if err = json.Unmarshal(result, &m); err != nil {
		glog.Errorf("Unable to Unmarshall the result from provider : %s", err)
	}
...
func writeShrinkResponse(w http.ResponseWriter, request models.StorageCluster, result []byte) {
	...
	if status != http.StatusOK {
		...
		for _, node := range request.Nodes {
			...
			coll := sessionCopy.DB(conf.SystemConfig.DBConfig.Database).C(models.COLL_NAME_STORAGE_NODES)
			if err := coll.Update(bson.M{"hostname": node}, bson.M{"$set": bson.M{"clusterid": "", "managedstate": models.NODE_STATE_FREE}}); err != nil {
				util.HttpResponse(w, http.StatusInternalServerError, err.Error())
				glog.Fatalf("Error updating the host details: %v", err)
				return
			}
...
func (a *App) Expand_Cluster(w http.ResponseWriter, r *http.Request) {
	...
	var route conf.Route
	provider := a.getProvider(body, route)
	provider.Client.Call(fmt.Sprintf("%s.%s", request.ClusterType, cluster_post_functions["expand"]), models.Args{Vars: mux.Vars(r), Request: body}, result)

	writeExpandResponse(w, request, result)
}
...
func (a *App) Forget_Cluster(w http.ResponseWriter, r *http.Request) {
	...
	if err := collection.Update(bson.M{"clusterid": cluster_id}, bson.M{"$set": bson.M{"$clusterid": ""}}); err != nil {
		util.HttpResponse(w, http.StatusInternalServerError, err.Error())
