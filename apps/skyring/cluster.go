...
func (a *App) POST_Clusters(w http.ResponseWriter, r *http.Request) {
	...
	provider.Client.Call(fmt.Sprintf("%s.%s", provider.Name, cluster_post_functions["create"]), models.Args{Vars: mux.Vars(r), Request: body}, &result)
...
func (a *App) Forget_Cluster(w http.ResponseWriter, r *http.Request) {
	...
	for _, node := range nodes {
		ret_val := GetCoreNodeManager().RejectNode(node.Hostname)
		if ret_val {
