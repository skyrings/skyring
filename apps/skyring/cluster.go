	...
	asyncTask := func(t *task.Task) {
		...
		for {
			...
			default:
				...
				t.UpdateStatus("Removing graphite whisper files of the cluster")
				if err := RemoveGraphiteWhisperFiles(ctxt, clusterName, cnodes); err != nil {
					util.FailTask(fmt.Sprintf("Error removing graphite whisper files of the cluster: %v", *uuid), fmt.Errorf("%s-%v", ctxt, err), t)
					if err := logAuditEvent(EventTypes["CLUSTER_FORGOT"],
						fmt.Sprintf("Failed to forget cluster: %s", clusterName),
						fmt.Sprintf("Failed to forget cluster: %s Error: %v", clusterName, err),
						uuid,
...
func (a *App) GET_ClusterNodes(w http.ResponseWriter, r *http.Request) {
	...
	if err := coll.Find(filter).All(&nodes); err != nil {
		if err == mgo.ErrNotFound {
			HttpResponse(w, http.StatusNotFound, fmt.Sprintf("Error getting the nodes for cluster: %v. error: %v", *cluster_id, err))
		} else {
...
func (a *App) GET_ClusterSlus(w http.ResponseWriter, r *http.Request) {
	...
	if err := coll.Find(filter).All(&slus); err != nil {
		if err == mgo.ErrNotFound {
			HttpResponse(w, http.StatusNotFound, fmt.Sprintf("Error getting the slus for cluster: %s. error: %v", cluster_id_str, err))
		} else {
...
func (a *App) GET_ClusterSlu(w http.ResponseWriter, r *http.Request) {
	...
	if err := coll.Find(bson.M{"clusterid": *cluster_id, "sluid": *slu_id}).One(&slu); err != nil {
		if err == mgo.ErrNotFound {
			HttpResponse(w, http.StatusNotFound, fmt.Sprintf("Error getting the slu: %v for cluster: %v. error: %v", *slu_id, *cluster_id, err))
		} else {
...
func (a *App) GET_ClusterConfig(w http.ResponseWriter, r *http.Request) {
	...
	if clusterFetchErr != nil {
		...
		if clusterFetchErr == mgo.ErrNotFound {
			HttpResponse(w, http.StatusNotFound, fmt.Sprintf("%s - Failed to fetch cluster %v.Error %v", ctxt, *cluster_id, clusterFetchErr))
		} else {
...
func RemoveGraphiteWhisperFiles(ctxt string, clusterName string, cnodes []models.Node) error {
