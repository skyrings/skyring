...
func (a *App) GET_NodeSummary(w http.ResponseWriter, r *http.Request) {
	...
	SLUDetails := SLU_Details(node_id, ctxt)
	var nodeSummary = map[string]interface{}{
		...
		"totalslu":    SLUDetails["TotalSLU"],
		"errorslu":    SLUDetails["ErrorSLU"],
		"warnslu":     SLUDetails["WarningSLU"],
		"downslu":     SLUDetails["DownSLU"],
	}
	json.NewEncoder(w).Encode(nodeSummary)
}

func SLU_Details(node_id *uuid.UUID, ctxt string) map[string]int {
	...
	for _, slu := range slus {
		...
		case slu.Status == models.SLU_STATUS_WARN:
			SLUDetails["WarningSLU"]++
		}
	}
	SLUDetails["TotalSLU"] = len(slus)
	return SLUDetails
}
