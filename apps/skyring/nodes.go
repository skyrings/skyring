...
func (a *App) GET_NodeSummary(w http.ResponseWriter, r *http.Request) {
	...
	if ServiceCount, err := GetCoreNodeManager().GetServiceCount(node.Hostname, ctxt); err != nil {
		logger.Get().Error(
		...
		for _, role := range node.Roles {
			switch role {
			case models.NODE_TYPE_OSD:
				SluUp := ServiceCount[models.SLU_SERVICE_COUNT]
				SluCount := nodeSummary[models.COLL_NAME_STORAGE_LOGICAL_UNITS].(map[string]int)
				SluDown := SluCount[models.TotalSLU] - ServiceCount[models.SLU_SERVICE_COUNT]
				ServiceDetails["slu"] = map[string]int{"up": SluUp, "down": SluDown}

			case models.Mon:
				ServiceDetails["mon"] = ServiceCount[models.MON_SERVICE_COUNT]
