...
func acceptNode(w http.ResponseWriter, request models.AddStorageNodeRequest) {
	ret_val := GetCoreNodeManager().AcceptNode(request.Hostname, request.SshFingerprint)
	if ret_val == true {
...
func addStorageNodeToDB(w http.ResponseWriter, r models.AddStorageNodeRequest) {
	...
	fmt.Println(storage_node.MachineId)
	...
	if storage_node.MachineId != "" && len(storage_node.NetworkInfo.Subnet) != 0 && len(storage_node.StorageDisks) != 0 {
		...
		util.HttpResponse(w, http.StatusInternalServerError, "Error fetching node details")
	}
}

func valid(req models.AddStorageNodeRequest) bool {
	if req.Hostname == "" || req.SshFingerprint == "" {
		return false
	}
