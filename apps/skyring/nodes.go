		...
		for nodeIndex, node := range nodes {
			if nodes[nodeIndex].Hostname < rUtilizations[utilizationIndex].NodeName {
				nodesWithUtilizations = append(nodesWithUtilizations, models.NodeWithUtilization{
					...
					Tags:          node.Tags,
					ManagementIP4: node.ManagementIP4,
					ClusterIP4:    node.ClusterIP4,
					PublicIP4:     node.PublicIP4,
					ClusterId:     node.ClusterId,
					Location:      node.Location,
					Status:        node.Status,
					State:         node.State,
					AlmStatus:     node.AlmStatus,
					AlmCount:      node.AlmCount,
					Options:       node.Options,
					CPUs:          node.CPUs,
					NetworkInfo:   node.NetworkInfo,
					StorageDisks:  node.StorageDisks,
					Memory:        node.Memory,
					OS:            node.OS,
