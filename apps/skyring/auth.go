...
func (a *App) SetAuthRoutes(router *mux.Router, authReqdRouter *mux.Router) error {
	...
	authReqdRouter.
		...
		Path("/users/{username}").
		Name("GET_getUser").
		Handler(http.HandlerFunc(a.getUserHandler))
	authReqdRouter.
		Methods("POST").
		Path("/users").
		Name("POST_addUsers").
...
func (a *App) getUserHandler(rw http.ResponseWriter, req *http.Request) {

	vars := mux.Vars(req)

	user, err := a.authProvider.GetUser(vars["username"])
	if err != nil {
	...
	if err != nil {
		glog.Errorf("Unable marshal the User:%s", err)
		util.HandleHHttpError(rw, err)
		return
	}
	rw.Write(bytes)
