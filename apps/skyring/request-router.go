...
routed. All the generic cases are covered here. But if any od the new endpoints
...
func (a *App) getProvider(body []byte, routeCfg conf.Route) *Provider {
	...
	for _, provider := range a.providers {
		glog.Infof("provider:", provider)
		if strings.Contains(routeCfg.Pattern, provider.Name) {
			result = &provider
			return result
		}
	}
