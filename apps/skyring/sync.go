...
func sync_cluster_status(cluster models.Cluster, provider *Provider) (bool, error) {
	...
	if err != nil || result.Status.StatusCode != http.StatusOK {
		...
		defer sessionCopy.Close()
		coll := sessionCopy.DB(conf.SystemConfig.DBConfig.Database).C(models.COLL_NAME_STORAGE_CLUSTERS)
		if statusMsg == models.STATUS_UP {
			// Set the cluster status UP
			logger.Get().Info("Updating the status of the cluster: %s to %d", cluster.Name, models.CLUSTER_STATUS_ACTIVE_AND_AVAILABLE)
			if err := coll.Update(bson.M{"clusterid": cluster.ClusterId}, bson.M{"$set": bson.M{"status": models.CLUSTER_STATUS_ACTIVE_AND_AVAILABLE}}); err != nil {
				return false, err
			}
		} else {
			// Set the cluster status as DOWN
			logger.Get().Info("Updating the status of the cluster: %s to %d", cluster.Name, models.CLUSTER_STATUS_NOT_AVAILABLE)
			if err := coll.Update(bson.M{"clusterid": cluster.ClusterId}, bson.M{"$set": bson.M{"status": models.CLUSTER_STATUS_NOT_AVAILABLE}}); err != nil {
				return false, err
			}
		}
	}
