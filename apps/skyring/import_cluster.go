	...
	asyncTask := func(t *task.Task) {
		for {
			...
			default:
				...
				t.UpdateStatus("Checking/accepting bootstrap node")
				...
				t.UpdateStatus("Getting nodes for the cluster")
				err = provider.Client.Call(
					fmt.Sprintf("%s.%s", provider.Name, "GetClusterNodes"),
					models.RpcRequest{
						RpcRequestVars:    vars,
						RpcRequestData:    []byte{},
				...
				if err != nil || result.Status.StatusCode != http.StatusOK {
					util.FailTask(
						fmt.Sprintf("%s-", ctxt),
						fmt.Errorf("Error getting nodes for cluster import"),
						t)
					return
				} else {
					var nodes []string
					if err := json.Unmarshal(result.Data.Result, &nodes); err != nil {
						util.FailTask(
							fmt.Sprintf("%s-", ctxt),
							fmt.Errorf("Error un-marshalling node details"),
							t)
						return
					}
					var failedNodes []string
					for _, node := range nodes {
						t.UpdateStatus(fmt.Sprintf("Checking/accepting node %s", node))
						if ok, err := checkAndAcceptNode(node, ctxt); err != nil || !ok {
							failedNodes = append(failedNodes, node)
						}
					}
					if len(failedNodes) > 0 {
						util.FailTask(
							"Failed to accept nodes: %v",
							fmt.Errorf("%s-", ctxt),
							t)
						return
					}
...
func checkAndAcceptNode(node string, ctxt string) (bool, error) {
	...
	if err := coll.Find(bson.M{"hostname": node}).One(&fetchedNode); err != nil {
		return false, fmt.Errorf(
			"Node %s is not boot-strapped properly. error: %v",
			node,
			err)
	} else {
