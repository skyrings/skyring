...
func (a *App) MonitorCluster(params map[string]interface{}) {
	...
	UpdateMetricToTimeSeriesDb(ctxt, disk_reads+disk_writes, time_stamp_str, fmt.Sprintf("%s%s-%s+%s", table_name, monitoring.DISK, monitoring.READ, monitoring.WRITE))
	...
	UpdateMetricToTimeSeriesDb(ctxt, cluster_interface_rx+cluster_interface_tx, time_stamp_str, fmt.Sprintf("%s%s-%s+%s", table_name, monitoring.INTERFACE, monitoring.RX, monitoring.TX))
...
func ComputeSystemSummary(p map[string]interface{}) {
	...
	*/
	...
	UpdateMetricToTimeSeriesDb(ctxt, netDiskRead+netDiskWrite, time_stamp_str, fmt.Sprintf("%s%s-%s+%s", table_name, monitoring.DISK, monitoring.READ, monitoring.WRITE))
	...
	UpdateMetricToTimeSeriesDb(ctxt, netIStatRx+netIStatTx, time_stamp_str, fmt.Sprintf("%s%s-%s+%s", table_name, monitoring.INTERFACE, monitoring.RX, monitoring.TX))
