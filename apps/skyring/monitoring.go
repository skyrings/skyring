...
func (a *App) Get_Summary(w http.ResponseWriter, r *http.Request) {
	...
	if err := coll.Find(bson.M{"name": monitoring.SYSTEM}).One(&system); err != nil {
		HttpResponse(w, http.StatusInternalServerError, fmt.Sprintf("%s - Could not fetch summary.Err %v", ctxt, err))
...
func Compute_System_Summary(p map[string]interface{}) {
	...
	if userNameErr != nil {
		logger.Get().Error("Error Creating the context. error: %v", userNameErr)
		return
	}
	ctxt := fmt.Sprintf("%v:%v", userName, reqId.String())
