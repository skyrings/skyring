...
func (a *App) Get_ClusterSummary(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	cSummary.IsNearFull = len(clusterThresholdEvenstInDb) > 0
	...
	for _, cluster := range clusters {
		...
		*/
		for profile, profileUtilization := range cluster.StorageProfileUsage {
			...
			net_storage_profile_utilization[profile] = map[string]interface{}{"utilization": {Used: used, Total: total, PercentUsed: percentUsed}}
		}
