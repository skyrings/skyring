...
func (a *App) MonitorCluster(params map[string]interface{}) {
	...
	if disk_reads_count > 0 && disk_writes_count > 0 {
		UpdateMetricToTimeSeriesDb(ctxt, math.Ceil(disk_reads+disk_writes), time_stamp_str, fmt.Sprintf("%s%s-%s_%s", table_name, monitoring.DISK, monitoring.READ, monitoring.WRITE))
	}
...
func ComputeSystemSummary(p map[string]interface{}) {
	...
	if len(clusters) > 0 {
		...
		if netDiskReadCount > 0 && netDiskWriteCount > 0 {
			UpdateMetricToTimeSeriesDb(ctxt, math.Ceil(netDiskRead+netDiskWrite), time_stamp_str, fmt.Sprintf("%s%s-%s_%s", table_name, monitoring.DISK, monitoring.READ, monitoring.WRITE))
		}
