...
func RemoveTimeSeriesFiles(ctxt string, clusterName string, cnodes []models.Node) error {
	...
	for _, node := range cnodes {
		...
		if err := os.RemoveAll(filepath.Join(conf.SystemConfig.TimeSeriesDBConfig.GraphiteDataRoot, hostname)); err != nil && !os.IsNotExist(err) {
			return err
		}
	}
	//Removing cluster graphite whisper files
	if err := os.RemoveAll(filepath.Join(conf.SystemConfig.TimeSeriesDBConfig.GraphiteDataRoot, clusterName)); err != nil && !os.IsNotExist(err) {
		return err
	}
	return nil
}
