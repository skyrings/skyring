	...
	for _, node := range nodes {
		...
		*/
		...
		for _, disk := range node.StorageDisks {
			...
			resource_name = monitoring.DISK + "-" + disk_name + monitoring.DISK_OPS
...
func (a *App) Get_ClusterSummary(w http.ResponseWriter, r *http.Request) {
	...
	cluster, clusterFetchErr := GetCluster(cluster_id)
	if clusterFetchErr != nil {
		logger.Get().Error("Unknown cluster with id %v.Err %v", entity_id, clusterFetchErr)
		HttpResponse(w, http.StatusInternalServerError, fmt.Sprintf("Unknown cluster with id %v.Err %v", entity_id, clusterFetchErr))
		return
	}
...
func Compute_System_Summary(p map[string]interface{}) {
	...
	*/
	...
	for _, cluster := range clusters {
		...
		*/
		...
		/*
			Calculate cpu user utilization
		*/
		cpuStat, cpuStatFetchError := GetMonitoringManager().GetInstantValue(cluster.Name, GetMonitoringManager().GetResourceName(map[string]interface{}{"resource_name": monitoring.CPU_USER}))
		if cpuStatFetchError != nil {
	...
	if err := GetMonitoringManager().PushToDb(map[string]map[string]string{table_name + GetMonitoringManager().GetResourceName(map[string]interface{}{"resource_name": monitoring.CPU_USER}): {time_stamp_str: strconv.FormatFloat(cluster_cpu_user/float64(len(clusters)), 'E', -1, 64)}}, hostname, port); err != nil {
		logger.Get().Error("%s - Error pushing cluster cpu utilization.Err %v", ctxt, err)
	}
