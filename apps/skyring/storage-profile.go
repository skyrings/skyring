...
func (a *App) GET_Cluster_Storageprofiles(w http.ResponseWriter, r *http.Request) {
}
...
func AddDefaultProfiles() error {
	//sas
	if err := GetDbProvider().StorageProfileInterface().SaveStorageProfile(models.StorageProfile{Name: models.DefaultProfile1, Priority: models.DefaultPriority}); err != nil {
		logger.Get().Error("Default Storage profile add failed: %v", err)
	}
	//ssd
	if err := GetDbProvider().StorageProfileInterface().SaveStorageProfile(models.StorageProfile{Name: models.DefaultProfile2, Priority: models.DefaultPriority}); err != nil {
		logger.Get().Error("Default Storage profile add failed: %v", err)
	}
	//general
	if err := GetDbProvider().StorageProfileInterface().SaveStorageProfile(models.StorageProfile{Name: models.DefaultProfile3, Priority: models.DefaultPriority}); err != nil {
		logger.Get().Error("Default Storage profile add failed: %v", err)
	}
	return nil
}
