...
func Initialize(node string) error {
	sessionCopy := db.GetDatastore().Copy()
	defer sessionCopy.Close()
	coll := sessionCopy.DB(conf.SystemConfig.DBConfig.Database).C(models.COLL_NAME_STORAGE_NODES)
	...
	asyncTask := func(t *task.Task) {
		for {
			...
			default:
				...
				// Process the request
				if err := initializeStorageNode(storage_node.Hostname, t); err != nil {
	...
	if taskId, err := GetApp().GetTaskManager().Run(fmt.Sprintf("Initialize Node: %s", storage_node.Hostname), asyncTask, 600*time.Second, nil, nil, nil); err != nil {
		logger.Get().Error("Unable to create the task for Initialize Node: %s. error: %v", storage_node.Hostname, err)
		return err
	} else {
