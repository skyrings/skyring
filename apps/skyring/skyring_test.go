...
func Test_StartProviders(t *testing.T) {
	...
	RequestStartProviderCodec = func(path string, confStr []byte) (*rpc.Client, error) {
		var temp *rpc.Client
		return temp, err
	}
	test := &App{}
	...
	test.StartProviders(configDir, binDir)
	testassert.AssertEqual(t, len(test.providers), 0, "None of the providers are initialized successfully")
	...
	test.routes = make(map[string]conf.Route)
	//call actual function wants to Test
	testassert.AssertNotEqual(t, len(test.providers), 0, "None of the providers are initialized successfully")
	...
	test.routes = make(map[string]conf.Route)
	//call actual function wants to Test
	testassert.AssertEqual(t, len(test.providers), 2, "Duplication of providers not handled")
}
