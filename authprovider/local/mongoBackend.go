...
func NewMongodbBackend(mongoURL string, database string) (b MongodbAuthBackend, e error) {
	...
	err = session.Ping()
	if err != nil {
		glog.Errorf("DB not responding:%s", err)
		return b, mkmgoerror(err.Error())
	}
	...
	if err != nil {
		glog.Errorf("Error creating session:%s", err)
		return b, mkmgoerror(err.Error())
	}
	b.session = session
	return
...
func (b MongodbAuthBackend) User(username string) (user model.User, e error) {
	c := b.connect()
...
func (b MongodbAuthBackend) Users() (us []model.User, e error) {
	c := b.connect()
	defer c.Database.Session.Close()
...
func (b MongodbAuthBackend) SaveUser(user model.User) error {
	c := b.connect()
	defer c.Database.Session.Close()
