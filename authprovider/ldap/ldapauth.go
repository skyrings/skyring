...
func (a Authorizer) GetDirectory() models.Directory {
	directory := models.Directory{}
	session := db.GetDatastore()
	c := session.DB("skyring").C("ldap")
	val, err := c.Count()
...
func (a Authorizer) ListExternalUsers(search string, page, count int) (users []models.User, err error) {
	...
	if directory.DomainAdmin != "" {
		block, err := aes.NewCipher([]byte("Skyring - RedHat"))
		if err != nil {
...
func (a Authorizer) SetDirectory(directory models.Directory) error {
	if directory.LdapServer == "" {
		logger.Get().Error("no directory server name given")
		return mkerror("no directory server name given")
	}
	if directory.Port == 0 {
		logger.Get().Error("no directory server port number given")
		return mkerror("no directory server port number given")
	}
	if directory.Base == "" {
		logger.Get().Error("no directory server connection string given")
		return mkerror("no directory server connection string given")
	}
