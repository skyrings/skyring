...
_logName = os.path.basename(sys.argv[0])


def InitLog(filename, logToStderr, level):
    logConfig = {'formatters': {'default': {'format': _formatString,
                                            'datefmt': '%Y-%m-%dT%H:%M:%S%z'}},
                 'handlers': {},
                 'loggers': {_logName: {'handlers': [],
                                       'propagate': False}},
                 'root': {'handlers': [], 'level': ''},
                 'version': 1}
    ...
    if filename:
        logConfig['handlers'].update({'file': {'class': 'logging.FileHandler',
                                               'formatter': 'default',
                                               'filename': (filename)}})
        logConfig['loggers'][_logName]['handlers'].append('file')
    ...
    if logToStderr:
        ...
        logConfig['loggers'][_logName]['handlers'].append('stderr')
