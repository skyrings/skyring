...
func (s Salt) AcceptNode(node string, fingerprint string, include_rejected bool) (status bool, err error) {
	if pyobj, err := pyFuncs["AcceptNode"].Call(node, fingerprint, include_rejected); err == nil {
...
func (s Salt) DisableServices(node string, services []string, stop bool) (status bool, err error) {
	if pyobj, err := pyFuncs["DisableServices"].Call(node, services, stop); err == nil {
		err = gopy.Convert(pyobj, &status)
	}
	return
}

func (s Salt) EnableServices(node string, services []string, start bool) (status bool, error error) {
	if pyobj, err := pyFuncs["EnableServices"].Call(node, services, start); err == nil {
		err = gopy.Convert(pyobj, &status)
	}
	return
}
