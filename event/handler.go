...
func initializeStorageNode(node string, t *task.Task) error {
	...
	if storage_node, ok := saltnodemanager.GetStorageNodeInstance(node); ok {
		if err := skyring.ApplyStorageProfiles(storage_node); err != nil {
			logger.Get().Error("Error applying storage profiles to node: %s Error: %v ", node, err)
			t.UpdateStatus("Error applying storage profiles to node: %s Error: %v ", node, err)
			if err := coll.Update(bson.M{"hostname": node}, bson.M{"$set": bson.M{"state": models.NODE_STATE_FAILED}}); err != nil {
				logger.Get().Critical("Error updating the node state for node: %s. error: %v", node, err)
				t.UpdateStatus("Error updating the node state for node: %s. error: %v", node, err)
				return err
			}
			return err
		}
