...
func collectd_status_handler(event models.AppEvent, ctxt string) (models.AppEvent, error) {
	...
	if err := UpdateNodeServiceState(event.NodeId, "collectd", active, ctxt); err!=nil {
...
func node_appeared_handler(event models.AppEvent, ctxt string) (models.AppEvent, error) {
	...
	if err := UpdateNodeServiceState(event.NodeId, "salt-minion", true, ctxt); err!=nil {
		logger.Get().Error("%s- Error updating service state for node: %s. Error:",ctxt,event.NodeId,event.NodeName)
		return event, err
	}
...
func node_lost_handler(event models.AppEvent, ctxt string) (models.AppEvent, error) {
	...
	if err := UpdateNodeServiceState(event.NodeId, "salt-minion", false, ctxt); err!=nil {
		logger.Get().Error("%s- Error updating service state for node: %s. Error:",ctxt,event.NodeId,event.NodeName)
		return event, err
	}
