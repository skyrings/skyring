...
func GetMetricValueFromCollectd(nodes []string, metricName string) (map[string]map[string]string, error) {
	result, err := salt_backend.UpdateMonitoringConfiguration(nodes, config, ctxt)
	return result, err
}
