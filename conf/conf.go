...
func LoadAppConfiguration(configFilePath string) *SkyringCollection {
	...
	if err != nil {
		glog.Fatalf("Error Reading SkyRing Config:", err)
		panic(fmt.Sprintf("Error Reading SkyRing Config:", err))
	}
	err = json.Unmarshal(file, &data)
	if err != nil {
		glog.Fatalf("Error Unmarshalling SkyRing Config:", err)
		panic(fmt.Sprintf("Error Unmarshalling SkyRing Config:", err))
	}
	return &data
