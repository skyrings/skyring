...
func (f *PyFunction) call(args ...interface{}) (*python.PyObject, error) {
	l := len(args)
	...
	if py_out == nil {
		return nil, errors.New("function failed at python side")
	}
...
func PyAddNode(node string, fingerprint string, username string, password string, master string) bool {
	py_out, _ := py_functions["add_node"].call(node, fingerprint, username, password, master)
	return py_out.IsTrue()
}

func PyGetManagedNodes() map[string][]string {
	py_out, _ := py_functions["get_managed_nodes"].call()
...
func PyGetNodeMachineId(node string) string {
	py_out, _ := py_functions["get_node_machine_id"].call(node)
	return python.PyString_AsString(py_out)
}

func PyGetNodeNetworkInfo(node string) map[string][]string {
	py_out, _ := py_functions["get_node_network_info"].call(node)
...
func PyGetNodeDiskInfo(node string) map[string]map[string]string {
	py_out, _ := py_functions["get_node_disk_info"].call(node)
