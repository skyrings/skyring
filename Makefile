...
VERSION   := 0.0.1
RELEASE   := 0.1
TARDIR    := skyring-$(VERSION)-$(RELEASE)
...
rpm:    build
	...
	cd $(SKYRING_BUILD_SRC); \
	tar -zcf skyring-$(VERSION)-$(RELEASE).tar.gz $(SKYRING_BUILD_SRC)/../$(TARDIR); \
	cp $(SKYRING_BUILD_SRC)/skyring-$(VERSION)-$(RELEASE).tar.gz $(RPMBUILD)/SOURCES; \
	rpmbuild -ba skyring.spec
	$(PRINT_STATUS); \
	if [ "$$EC" -eq "0" ]; then \
		FILE=$$(readlink -f $$(find $(RPMBUILD)/RPMS -name skyring-$(VERSION)-$(RELEASE)*.rpm)); \
